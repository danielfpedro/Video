...
class AlController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Al->save($al)) {
                $this->Flash->success('The al has been saved.');
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error('The al could not be saved. Please, try again.');
            }
        }
        $this->set(compact('al'));
        $this->set('_serialize', ['al']);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Al->delete($al)) {
            $this->Flash->success('The al has been deleted.');
        } else {
            $this->Flash->error('The al could not be deleted. Please, try again.');
        }
        return $this->redirect(['action' => 'index']);
    }
}
